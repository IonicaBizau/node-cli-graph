 ...
 * @return {CliGraph} The CliGraph instance.
 */
CliGraph.prototype.setFunctionX = function (foo, min, max) {
    var self = this;

    min = min === undefined ? - (self.options.width + self.options.center.x) / 2 : min;
    max = max === undefined ? (self.options.width + self.options.center.x) / 2 : max;
 ...
 * @return {CliGraph} The CliGraph instance.
 */
CliGraph.prototype.setFunctionY = function (foo, min, max) {
    var self = this;

    min = min === undefined ? - (self.options.height + self.options.center.y) / 2 : min;
    max = max === undefined ? (self.options.height + self.options.center.y) / 2 : max;

    for (var i = min; i <= max; ++i) {
        self.addPoint(foo(i), i);
    }
